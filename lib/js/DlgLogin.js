        ...
        success: function (json) {
            ...
            for (var i = 0; i < friends.length; i++) {
                $('ul#friends_list').append(function () {
                    return '<li class="nav-item" id="friends_' + friends[i]["user2"] + '">' +
                        '<a class="nav-link" href="#">' +
                        '<span data-feather="file-text">' + friends[i]["user2"] + '</span><br>' +
                        '</a>' +
                        '</li>' +
                        '<br>';
                });
...
Dlglogin.prototype._add_friends = function () {
    var me = this;
    $("button#add_friends").click(function (e) {
        e.preventDefault();
        if ($("input#search_friend").val() == "") {
            alert("查找好友不能为空！");
        } else {
            Connector({
                data: {
                    ...
                    params: {
                        username: username,
                        friends: $("input#search_friend").val()
                    }
                },
